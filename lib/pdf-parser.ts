...
const NUM_TOKEN = /\b-?(?:\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d{1,3}(?:\.\d{3})+(?:,\d+)?|\d+[.,]\d+|\d+)\b/g;


function toDotDecimal(raw: string): string {
  // Quita miles y deja 2 decimales con punto
  let t = raw.replace(/\s+/g, "");
  // Si hay . y , el decimal es el último símbolo que aparezca
  const lastDot = t.lastIndexOf(".");
  const lastComma = t.lastIndexOf(",");
  const decimalIsComma = lastComma > lastDot;

  // Limpia miles y normaliza decimal a '.'
  if (decimalIsComma) {
...
export async function parsePdfReceiptToRecord(file: File): Promise<Parsed> {
  ...
  const ok = await fetch("/pdf.worker.min.mjs", { method: "HEAD" }).then(r => r.ok).catch(() => false);
  ...
  for (let i = 1; i <= (pdf.numPages ?? 1); i++) {
    ...
    const words: Word[] = (content.items as any[])
      .map((it: any) => {
        ...
        return { str: String(it.str ?? "").trim(), x: Number(tr[4] || 0), y: Number(tr[5] || 0) };
      })
  ...
  // ---------- PERIODO: "la fecha por encima de LEGAJO" ----------
  ...
  if (legLine > 0) {
    // busca mm/yyyy en las 3 líneas de arriba (de más cercanas a más lejanas)
  ...
  // Si no encontramos arriba de LEGAJO, usar "cerca de Período" o, en último caso, el más reciente global
  if (!per) {
    const near: string[] = [];
    for (let li = 0; li < allLines.length; li++) {
      const joined = allLines[li].map((w) => w.str).join(" ");
    ...
    per = pickMostRecent(near) ?? pickMostRecent([...rawText.matchAll(MMYYYY_ALL)].map(m => m[1]));
  }
  if (per) data.PERIODO = per;

  // ---------- LEGAJO ----------
  if (legLine >= 0) {
    // misma línea o siguientes 4 líneas
    const searchLines = [allLines[legLine], allLines[legLine + 1], allLines[legLine + 2], allLines[legLine + 3], allLines[legLine + 4]].filter(Boolean) as Word[][];
  ...
  for (const line of allLines) {
    // tokens numéricos (guardamos su X para elegir el más a la derecha)
    ...
    // detectar códigos (acepta "20 500")
    ...
    // helpers para elegir el importe correcto
    const hasDecimals = (s: string) => /[.,]\d{1,}$/.test(s);
    const digitsCount = (s: string) => s.replace(/\D/g, "").length;

    for (const code of codes) {
      if (!numTokens.length) { data[code] = "-"; continue; }

      // 1) tokens con decimales
      const decimalTokens = numTokens.filter(t => hasDecimals(t.raw));

      // 2) de los decimales, preferir los que claramente son importes (entera con >=3-4 dígitos)
      const decimalBig = decimalTokens
        .filter(t => {
          // cuenta dígitos de la parte entera
          ...
          return intPart.length >= 3; // >=3 dígitos en la entera (típico de importes)
        })
        .sort((a, b) => a.x - b.x);

      // 3) elegimos el MÁS A LA DERECHA
      let chosenRaw: string | null = null;
      if (decimalBig.length) {
        chosenRaw = decimalBig[decimalBig.length - 1].raw;
      } else if (decimalTokens.length) {
        chosenRaw = decimalTokens[decimalTokens.length - 1].raw;
      } else {
        // 4) sin decimales: último número de la línea
        chosenRaw = numTokens[numTokens.length - 1].raw;
      }

      data[code] = toDotDecimal(chosenRaw!); // ← sin miles, con 2 decimales
    }
  }
  ...
  // console.log("PARSER → debugLines:", debugLines.length);
